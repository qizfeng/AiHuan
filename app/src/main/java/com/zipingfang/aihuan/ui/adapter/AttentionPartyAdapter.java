 ...
 */
public class AttentionPartyAdapter extends BaseAdapter {
    private Context mContext;
    private List<Attention> mData = new ArrayList<Attention>();
    private LayoutInflater mInflater;
    private String mid;

    public AttentionPartyAdapter(Context mContext, List<Attention> mData, String mid) {
...
//        }
       ...
       final long  time = Long.parseLong(item.getLast_time());
        if ("已结束".equals(item.getIs_start())) {
            holder.tv_time.setText("专场已结束");
        } else if ("已开始".equals(item.getIs_start())) {
        ...
        } else if ("未开始".equals(item.getIs_start())) {
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    long curTime = Long.parseLong(item.getLast_time())-1;
            ...
            public void onClick(View v) {
                ...
                builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        serverDao.mid = mid;
                        serverDao.party_id = item.getParty_id();
                        serverDao.party_type = item.getParty_type();
                        serverDao.type = 1;
