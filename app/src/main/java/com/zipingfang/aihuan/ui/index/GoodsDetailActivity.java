...
import com.zipingfang.aihuan.dao.GoodsDetailNormalServerDao;
...
//        webSettings.setBuiltInZoomControls(true);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.iv_minus:
                ...
                break;
            case R.id.iv_add:
                price = Float.parseFloat(et_price.getText().toString().trim());
                price=price+1;
                et_price.setText(price+"");
                break;
        }
    ...
    private void doAttention(){
        ...
        serverDao.loadData(new BaseDao.IDaoCallback() {
            ...
            public void exec(boolean success, Object obj) {
                if(serverDao.isSucc){
                    ToastUtils.show(GoodsDetailActivity.this,"关注成功");
                }else {
                    ToastUtils.show(GoodsDetailActivity.this,serverDao.desc);
                }
            }
        });
    }
    ...
    private void loadData() {
        agid = getIntent().getStringExtra("agid");
        pid = getIntent().getStringExtra("party_id");
        final GoodsDetailNormalServerDao serverDao = new GoodsDetailNormalServerDao(this);
        serverDao.goods_id= agid;
        serverDao.loadData(new BaseDao.IDaoCallback() {
            @Override
            public void exec(boolean success, Object obj) {
                if (serverDao.isSucc) {
...
//        time = Long.parseLong(goodsDetail.getLast_time());
        ...
        tv_price_type.setText("促销价");
        tv_goods_now_price.setText("￥" + goodsDetail.getStarting_price());
        et_price.setText(goodsDetail.getStarting_price());
        tv_apply.setText("限购 " + goodsDetail.getSale_number());
        tv_hot.setText("售出 " + goodsDetail.getSale_number());
